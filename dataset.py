...
class VideoClipDataset(Dataset):
    def __init__(self, root_dir_path=None, EncodeAndTransformedVideo_obj=None, GetSpectrogramFromAudio_obj=None, GetTextFromAudio_obj=None): 
        """
        ...
        self.videos = glob.glob(os.path.join(self.root_dir_path,'*/*'))

    def __getitem__(self, index):
        video = self.videos[i]
        transformed_video = EncodeAndTransformedVideo_obj.get_video(video) #get_video (or any func name): function defined by Raghav in his class to get the encoded and transformed video
        ...
        processed_speech = GetTextFromAudio_obj.get_speech(video) #get_speech (or any func name): function defined by Joon in his class to get the transcribed speech
